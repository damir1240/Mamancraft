...
VulkanPipeline::VulkanPipeline(const std::unique_ptr<VulkanDevice> &device,
  ...
  if (configInfo.renderPass == VK_NULL_HANDLE) {
    MC_CRITICAL("Cannot create graphics pipeline with a null renderPass!");
    throw std::runtime_error(
        "Cannot create graphics pipeline with a null renderPass!");
  }
  ...
  if (vkCreateGraphicsPipelines(m_Device->GetLogicalDevice(), VK_NULL_HANDLE, 1,
                                &pipelineInfo, nullptr,
                                &m_GraphicsPipeline) != VK_SUCCESS) {
    MC_CRITICAL("Failed to create Vulkan Graphics Pipeline!");
...
void VulkanPipeline::CreatePipelineLayout() {
  ...
  if (vkCreatePipelineLayout(m_Device->GetLogicalDevice(), &pipelineLayoutInfo,
                             nullptr, &m_PipelineLayout) != VK_SUCCESS) {
    MC_CRITICAL("Failed to create Vulkan Pipeline Layout!");
...
void VulkanPipeline::DefaultPipelineConfigInfo(PipelineConfigInfo &configInfo) {
  ...
  configInfo.rasterizationInfo.cullMode =
      VK_CULL_MODE_NONE; // We will use NONE for now so we see the triangle
                         // anyway
